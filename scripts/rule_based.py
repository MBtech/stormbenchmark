...
def hill_climbing(conf,sample,start,end,step,typ, relations,basefile, metric,lat_p,tp_p,behav_tp,behav_lat):
    ...
    while max_capacity<0.8:    
        ...
        config = downgrade(capacity,config,["component.rolling_count_bolt_num","component.split_bolt_num"],step)
        design_space.append(config)
        metric_values,numbers = get_results(length,length+1,design_space, basefile,metric)
    ...
    print index
    capacity = getstats(index)
    config = adjust(capacity,x0,["component.rolling_count_bolt_num","component.split_bolt_num"],step)
    design_space.append(config)
    metric_values,numbers = get_results(length,length+1,design_space, basefile,metric)
    fx0 = min(metric_values)
    x0 = dict(config) 
...
#        x0 = dict(x_new)
    ...
    while priority<t:
        ...
        x_new = step_func(x0,start,end,step,typ,p,c,relations,change[met[priority]])
        length = len(design_space)
        ...
        if fx_new<fx0:
            ...
            x0 = dict(x_new)
            continue
        elif constraint=="throughput" and tp>tp_old:
            fx0 = fx_new
            x0 = dict(x_new)
            priority+=1
        else:
