    ...
    for c in conf:
        if c in typ.keys():
            if typ[c] == "exp":
                ...
                start[c] = max(math.log(center[c],2)-round(r/2),start[c])
                end[c] = min(math.log(center[c],2)+round(r/2),end[c])
        else:
            r = int((end[c] - start[c])*alpha)
...
def get_results(start, end, design_space, basefile, metric):
    ...
    #else:
    #    return list([sys.maxint]),ret_array
...
def hill_climbing(conf,sample,start,end,step,typ, basefile, metric):
    ...
    n =2
    l =2
    #m = int(0.05*total_runs) # Initial LHS sample size
    ...
    while restart:
        ...
        while local_search:
            ...
            metric_values,numbers = get_results(length,length+n,design_space, basefile,metric)
        ...
        design_space.extend(generate_LHS(result,start,end,step,typ,p,conf,l))
        # Get results and get the best configuration
...
def main():
    ...
    hill_climbing(conf,sample,start,end,step,typ,basefile,metric)
